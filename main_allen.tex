...
with the training (frontal) images from the $i$-th class $A_i$. The algorithm 
extends Lucas-Kanade iterative alignment \cite{LucasB1981} using the
$\ell_1$-norm as a robust error function.  
...
workstation for security and access control applications, as shown in Figure \ref{fig:pipeline}. 
...
that ALM is a better choice among existing $\ell_1$-min
...
particularly suitable for systems with parallel-computing pipelines. In
...
programmable GPUs, which is similar to the hardware setup of this paper.
...
size and speed of on-chip memory (cache) have big performance implications.
...
sections \ref{sec:alignment} and \ref{sec:recognition}.

We will primarily compare CPU and GPU architectures on a per-board basis:
...
on-chip cache, and relatively high clock speed.

The baseline architecture for our experiment is a Linux workstation with two
...
Each core has a private 32\,KB L1 data cache and a private 256\,KB L2 cache (\ie, a total of 2\,MB L2 cache available for the eight cores).
Therefore, for the low-resolution images used in face recognition (say, on the order of $64\times64$ grayscale images), the L2 cache
in principle can hold up to 16 images per core and 128 images in total.
...
available for a dual-processor configuration, which corresponds to about 1024 images.  
...
(SSE) capable of performing the same arithmetic operation on four single-precision floating point values simultaneously.  
...
on-chip cache, and relatively low clock speed.  

Compared to CPU architectures, modern GPU architectures dedicate less of their transistors
to cache, and more of them for arithmetic logic. Our target GPU architecture
is the Nvidia Fermi architecture (\eg, GTX 480 GPU used in this paper). CUDA based GPUs are comprised of several 
\emph{streaming multiprocessors} (SMP) (currently 14 to 16 for a high end GPU).
...
higher levels called \emph{thread blocks} and \emph{grids}.  Since SMPs are not designed
to communicate between themselves during GPU program execution, tasks
are typically broken up into multiple GPU programs (kernels) that are executed
serially.\footnote{Please add a footnote to further explain.}
...
Compared to typical CPUs, GPUs have much higher peak computational bandwidth
as well as data bandwidth to DRAM.  However, since our system primarily consists
of matrix-vector operations and image resampling operations, it has a very low
arithmetic intensity (\ie, we perform a very small number of arithmetic
