...
hardware capabilities, and refer the reader to the NVIDIA C for CUDA Programming Guide.

Our target GPU architecture
is the Nvidia Fermi architecture (\eg, the GTX 480 GPU used in this paper). 
CUDA based GPUs are comprised of several \emph{streaming multiprocessors} (SMP) 
(currently 14 to 16 for a high end GPU).  Each SMP is capable of executing up to 64
floating point operations concurrently.  

%The GPU uses a SPMD (Single Program Multiple Data) programming model: at any given
%point in time, all threads are running the same gpu program, or \emph{kernel}, but
%they all operate on different data.  During kernel execution, threads can only communicate
%(or synchronize with each other) within groups of threads called blocks.  Each block
%is further subdivided into groups of 32 threads called \emph{warps}. 
%Typically, an SMP will hold several thread blocks at a time, although it "only" has enough 
%hardware to schedule
%2 warps to perform a floating point operation at a time (the remaining warps are either
%inactive, or waiting for data to arrive).  Threads within a block are able to synchronize
%(wait at certain points of a program), which gives them a chance to communicate via a
%relatively small amount of shared cache.
...
L2-cache.  For our system, each SMP has 64 KB of L1 cache for a total of 896
...
This section discusses how to efficently map the two levels of parallelism
available in iterative face alignment to the two levels of concurrency
available in the NVIDIA Fermi GPU architecture.
...
hardware will allow.  
This mapping of the alignment problem onto the concurrency provided by the pair
of CPUs is visualized in Figure \ref{fig:alignment_mapping_gpu}.
\begin{figure}
\centering
...
\subsection{Other Implementation Strategies}
Several GPU implementation strategies were explored during the course of our
development, and are only mentioned briefly.  Listed in order of increasing
performance based on a synthetic benchmark of the $\ell_1$ minimization step,
they included:
\begin{enumerate}
