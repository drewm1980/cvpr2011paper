...
$\bb$ based on the training image labels associated with $A$.  Since $A$
...
\cite{WagnerA2009-CVPR}, the framework of SBC was extended to align
a query image to each subject class individually. The face alignment algorithm iteratively solves
...
\end{figure}
The tiers seen in the GPU profile result from the GPU hardware scheduling the computation of alignment
...
%the alignment stage, and motivating a threshold on how many subjects are kept for
...
%threshold on potential recognition rate, we vary the number of subjects kept for
%the recognition stage and estimate the probability of the test user being
%among the set of subjects kept for the recognition stage.  This curve
...
%diminishing returns as the number of kept subjects is increased beyond 20; a small
...
%number of subjects kept for the recognition stage.}
