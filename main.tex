...
Due to the low arithmetic intensity of ALM solvers, implementations on both
architectures are primarily limited by memory bandwidth,  
...
naive library-based implementations. 
...
The $ell_1$ solvers in the original system prototype were based 
on standard interior-point methods \cite{WagnerA2009-CVPR}. Their speed has been improved by ALM in a more recent paper \cite{WagnerA2011-PAMI}. However, due the high per-class cost
of the alignment step, the recognition system still fails to achieve real-time
recognition against datasets of hundreds or thousands of subjects.  In this paper,
in addition to accelerating the generic
$\ell_1$-min objectives \eqref{eq:l1min} and \eqref{eq:l1min_denoise}, we 
also discuss how to efficiently accelerate the iterative face alignment step
...
applications targeting thousands of subjects or more, will likely require computation be
...
\eqref{eq:l1min_alignment} is impossible since it is a non-convex problem
...
library-based, and the optimized version we advocate.
...
both processors.  The potential advantage of this implementation is that all of the variables
for the inner loop, most notably $B_j$ and $B_j^\dagger$, fit in L2 cache.  

The optimized implementation maps problem-level parallelism onto core-level
...
For these reasons, we expect this implementation to out-perform
...
same purpose.
\footnote{In CUDA terms, our proposed alignment stage
...
but can be indirectly influenced by manual tuning of the code.\footnote{In CUDA terms, it is the kernel launch
configuration that is being tuned.} We have empirically determined
...
Second of all, since there are no standard CUDA libraries that work at the SMP level, we use a
custom routine for computing $B_j^\dagger = (B_j^TB_j)^{-1} B^T = G^{-1} B_j^T$. $G$
...
Similarly, reductions (i.e. sum or $\ell_1$ norm) on image-sized vectors are also
...
After the face alignment stage is complete, the 20 subjects with lowest
$||\e_i||_1$ are selected for recognition, $\bb$ and $A_i$ are re-sampled to a common alignment
...
alignment stage of the face recognition pipeline (i.e., Algorithm
\ref{alg:iterative_alignment}), as well as the recognition stage of the
face recognition pipeline (i.e., Algorithm \ref{alg:alm_rec}).
...
Compared to previously published results, the new implementation can align the query
image with 250 subject classes in under 10 seconds, while the best previous effort
required 1.7 seconds per subject class (i.e. 253 seconds for 250 subjects).
