...
$\ell_1$-min based on augmented Lagrangian method (ALM) solvers. We demonstrate
through extensive simulation and real-world experiment that the proposed
algorithms significantly speed up the sparsity recovery problem compared to the
state of the art.  Despite the low arithmetic intensity of ALM solvers, the GPU
can still outperform the CPU by a wide margin for large problem sizes, and when
solving many small problems simultaneously. The CPU implementation remains
faster for solving a single moderately sized problem. The source code of the
...
In this section, we briefly describe the ALM algorithm for $\ell_1$-min \eqref{eq:l1min} \cite{YangA2010-ICIP}
and analyze its complexity. Lagrange multipliers have been frequently used in convex programming to
eliminate equality constraints via adding a penalty term to the
...
Lagrange multipliers in an iterative fashion.  For $\ell_1$-min \eqref{eq:l1min}, the augmented
