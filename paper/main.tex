...
which is based on CUBLAS and Nvidia CUDA libraries, will be made available on our
...
Algorithm \ref{alg:alm} summarizes the ALM $\ell_1$-min algorithm \footnote{While we
...
that solve a single large instance of the $\ell_1$-minimization problem (Algorithm \ref{alg:alm_rec}) by
...
The ground truth signals, $\xx_0$ and $\yy_k$ had a sparsity rate of 0.1 with elements generated using the standard normal and normalized with unit norm.
Because the ground truth signal $\xx_0$ and $yy_k$ are known, the algorithm terminates
when $norm(\bx-\bx_0) < tolerance$.
The measurement vector was generated with the equation $\y_0 = A \x_0 + \y_k$.   
...
{\bf VS:  this accuracy number depends on the number of subjects in the A matrix right?  So we either 
need to fix the number of subjects to compare against or vary it...}

The third experiment is to compare accuracy rate of the face recognition algorithm at different
image resolutions.

The fourth experiment is to compare the speed of the recognition stage w.r.t the number of 
subjects in the matrix $A$.
