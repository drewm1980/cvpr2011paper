...
%The GPU contains an inverted memory hierarchy so that the L1 cache stays close to the cores, providing the increase in performance.  It essentially turns the GPU from being memory-limited to computational-limited.
...
%Each SMP is analogous to a single CPU except that it can run multiple threads simultaneously.
