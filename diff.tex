...
\title{\DIFaddbegin \vspace{-0.2in} \DIFaddend Parallelization of Fast $\ell_1$-Minimization for Face Recognition}
...
For very large problems\DIFaddbegin \DIFadd{, }\DIFaddend the GPU is faster \DIFaddbegin \DIFadd{due }\DIFaddend to higher memory bandwidth, while 
for problems that fit in the larger CPU L3 cache, the CPU is faster.
\DIFdelbegin \DIFdel{At the problem sizes most important for $\ell_1$-min based
face recognition, both architectures achieve comparable performance.
}\DIFdelend On both architectures, the proposed implementations significantly outperform
naive library-based implementations, as well as \DIFdelbegin \DIFdel{published results}\DIFdelend \DIFaddbegin \DIFadd{previous systems}\DIFaddend . 
...
\DIFdelbegin \DIFdel{the classical framework of
}\DIFdelend \emph{augmented Lagrangian methods} (ALM) \cite{BertsekasD2003,YangA2010-ICIP}.
...
\DIFdelbegin \DIFdel{The $ell_1$ solvers in the original system prototype were based 
on standard interior-point methods }%DIFDELCMD < \cite{WagnerA2009-CVPR}%%%
\DIFdel{. Their speed has been improved by ALM in a more recent paper }\DIFdelend \DIFaddbegin \DIFadd{Although ALM has been applied to improve the speed of a prototype face recognition system }\DIFaddend \cite{WagnerA2011-PAMI}\DIFdelbegin \DIFdel{. However,
due }\DIFdelend \DIFaddbegin \DIFadd{,
due to }\DIFaddend the high per-class cost
of the alignment step, the \DIFdelbegin \DIFdel{recognition }\DIFdelend system still fails to achieve \DIFdelbegin \DIFdel{real-time
recognition }\DIFdelend \DIFaddbegin \DIFadd{\emph{real-time}
performance }\DIFaddend against datasets of hundreds or thousands of subjects.  In this paper,
in addition to accelerating the generic
$\ell_1$-min objectives \eqref{eq:l1min} and \eqref{eq:l1min_denoise}, we 
also discuss how to efficiently accelerate the \DIFdelbegin \DIFdel{iterative }\DIFdelend face alignment step
...
applications targeting thousands of subjects or more \DIFdelbegin \DIFdel{, }\DIFdelend will likely require computation be
distributed over a cluster or network of computers\DIFdelbegin \DIFdel{, and this }\DIFdelend \DIFaddbegin \DIFadd{. This variant }\DIFaddend is not addressed here.
...
One of the drawbacks of most interior-point methods for $\ell_1$-min \DIFdelbegin \DIFdel{(including
BP, LASSO, and TNIPM) }\DIFdelend is that they require the solution sequence to follow an
...
speed of $\ell_1$-min \DIFdelbegin \DIFdel{. Homotopy methods for recovering sparse signals were
first studied in the context of LASSO }%DIFDELCMD < \cite{OsborneM2000}%%%
\DIFdel{, which inspired a
solution to the \emph{forward stagewise linear regression} problem called LARS
}%DIFDELCMD < \cite{EfronB2004}%%%
\DIFdel{, and eventually led to the corresponding Homotopy algorithms
for BP in }%DIFDELCMD < \cite{MalioutovD2005,DonohoD2006}%%%
\DIFdel{.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \cite{OsborneM2000,EfronB2004,MalioutovD2005,DonohoD2006}\DIFadd{.
}\DIFaddend Although Homotopy can be shown to exactly estimate BP when the solution is
...
%DIF > Homotopy methods for recovering sparse signals were
%DIF > first studied in the context of LASSO \cite{OsborneM2000}, which inspired a
%DIF > solution to the \emph{forward stagewise linear regression} problem called LARS
%DIF > \cite{EfronB2004}, and eventually led to the corresponding Homotopy algorithms
%DIF > for BP in \cite{MalioutovD2005,DonohoD2006}.


\DIFaddend %\footnote{During the face alignment algorithm presented later, The $A$ matrix changes between alignment iterations but not within the L1 solver}  
...
while achieving estimation accuracy competitive \DIFdelbegin \DIFdel{with exact }\DIFdelend \DIFaddbegin \DIFadd{to other }\DIFaddend $\ell_1$-min
solutions\DIFdelbegin \DIFdel{such as BP and Homotopy methods, and other IST-type
algorithms}\DIFdelend . Therefore, we choose ALM as the core algorithm for
...
\label{eq:alm}
\end{equation}
where $\mu_{k}\rightarrow \infty$ \DIFdelbegin \DIFdel{is monotonicallyincreasing}\DIFdelend \DIFaddbegin \DIFadd{increases monotonically}\DIFaddend .
...
memory, their sizes and \DIFdelbegin \DIFdel{speeds }\DIFdelend \DIFaddbegin \DIFadd{speed }\DIFaddend have a dramatic effect on performance.  We give a
...
For our GPU implementations\DIFaddbegin \DIFadd{, }\DIFaddend we target NVIDIA Fermi GPU architecture (\eg, the GTX 480
...
\eqref{eq:l1min_alignment} is \DIFdelbegin \DIFdel{impossible }\DIFdelend \DIFaddbegin \DIFadd{inefficient }\DIFaddend since it is a non-convex problem
...
In this paper\DIFaddbegin \DIFadd{, }\DIFaddend we compare two mappings of the parallelism available in the alignment
stage to the concurrency available on the CPU: a naive implementation that is purely 
\DIFdelbegin \DIFdel{library-based, and the }\DIFdelend \DIFaddbegin \DIFadd{based on stock libraries and compilers, and a manually }\DIFaddend optimized version we advocate.
...
other words, alignment \DIFdelbegin \DIFdel{is performed }\DIFdelend of the query image \DIFdelbegin \DIFdel{against the training images of }\DIFdelend \DIFaddbegin \DIFadd{is performed
against }\DIFaddend a single user at a time using all \DIFdelbegin \DIFdel{cores on
both }\DIFdelend \DIFaddbegin \DIFadd{available cores on
the }\DIFaddend processors.  The potential advantage of this implementation is that all of the variables
for the inner loop, most notably $B_j$ and $B_j^\dagger$, fit in L2 cache.  

\DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{Instead, our manually }\DIFaddend optimized implementation maps problem-level parallelism onto core-level
...
For these reasons, \DIFdelbegin \DIFdel{we expect this implementation to out-perform
the previous }\DIFdelend \DIFaddbegin \DIFadd{this }\DIFaddend implementation \DIFaddbegin \DIFadd{outperforms the previous naive solution}\DIFaddend .
...
cache usage, \DIFdelbegin \DIFdel{on the GPU }\DIFdelend cache is relatively unimportant \DIFaddbegin \DIFadd{on the GPU }\DIFaddend for ALM based
...
The strength of the GPU's memory architecture for our purposes is \DIFdelbegin \DIFdel{it's }\DIFdelend \DIFaddbegin \DIFadd{its }\DIFaddend ability to sustain a very
...
but can be indirectly influenced by manual tuning of the code \DIFdelbegin \DIFdel{.}\footnote{\DIFdel{In CUDA terms, it is the kernel launch
configuration that is being tuned.}} %DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{(i.e., the kernel launch
configuration in CUDA terms). }\DIFaddend We have empirically determined
...
Second\DIFdelbegin \DIFdel{of all}\DIFdelend , since there are no standard CUDA libraries that work at the SMP level, we use a
custom routine for computing $B_j^\dagger = (B_j^TB_j)^{-1} B^T = G^{-1} B_j^T$\DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{, while }\DIFaddend $G$
...
Similarly, \DIFdelbegin \DIFdel{reductions (i.e. sum or $\ell_1$ norm) on image-sized }\DIFdelend \DIFaddbegin \DIFadd{sums and dot-products of long }\DIFaddend vectors are also
...
After the face alignment stage is complete, \DIFdelbegin \DIFdel{the }\DIFdelend 20 \DIFdelbegin \DIFdel{subjects with lowest
$||\e_i||_1$ are }\DIFdelend \DIFaddbegin \DIFadd{subject classes with lowest
alignment error will be }\DIFaddend selected for recognition, $\bb$ and $A_i$ are re-sampled to a common alignment
...
vector is generated \DIFdelbegin \DIFdel{with the equation $\bb_0 = A \xx_0$}\DIFdelend \DIFaddbegin \DIFadd{by $\bb = A \xx_0$}\DIFaddend .   

The results of this benchmark can be found in Figure \ref{fig:random_data}.
The \DIFdelbegin \DIFdel{x-axis }\DIFdelend \DIFaddbegin \DIFadd{$x$-axis }\DIFaddend represents the size of the $A$ matrix and the \DIFdelbegin \DIFdel{y-axis }\DIFdelend \DIFaddbegin \DIFadd{$y$-axis }\DIFaddend represents the
average amount of time \DIFdelbegin \DIFdel{it }\DIFdelend to complete a single problem instance.  The GPU
...
single small problem.  The transition between the two regimes occurs \DIFaddbegin \DIFadd{(at 2000) }\DIFaddend where
...
\label{sec:benchmark}
This section presents benchmarks of \DIFdelbegin \DIFdel{our }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend CPU and GPU implementations of the
alignment stage \DIFdelbegin \DIFdel{of the face recognition pipeline }\DIFdelend (i.e., Algorithm
\ref{alg:iterative_alignment}) \DIFdelbegin \DIFdel{, }\DIFdelend as well as the recognition stage \DIFdelbegin \DIFdel{of the
face recognition pipeline }\DIFdelend (i.e., Algorithm \ref{alg:alm_rec}) \DIFaddbegin \DIFadd{of the
face recognition pipeline}\DIFaddend .
...
seeks a similarity transformation between the coordinate frame of the \DIFdelbegin \DIFdel{"camera"
resolution test image
}\DIFdelend \DIFaddbegin \DIFadd{test image
resolution }\DIFaddend and the coordinate frame of the \DIFdelbegin \DIFdel{"canonical" resolution
images }\DIFdelend \DIFaddbegin \DIFadd{``canonical'' resolution
}\DIFaddend which the images are compared. We use a $64 \times 64$ pixel window in
the canonical frame. The training images are aligned by manually clicking the
two outer eye corners in the camera frame, and then warped via a
similarity transformation such that the eye corners lie at a pair of canonical
eye locations in the canonical frame.
...
\begin{figure}[t!]
\centering
\includegraphics[width=3in]{figures/alignment_runtime_graph}\vspace{0.05in}
...
achievable given the alignment \DIFdelbegin \DIFdel{results}\DIFdelend \DIFaddbegin \DIFadd{selection process}\DIFaddend .  For significantly lower resolutions,
